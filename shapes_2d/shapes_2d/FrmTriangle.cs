...
namespace shapes_2d
{
    public partial class FrmTriangle : Form
    {
        ...
        {
            bool tieneAncho = double.TryParse(TxtAncho.Text, out double a) && a > 0;
            bool tieneLargo = double.TryParse(TxtLargo.Text, out double b) && b > 0;
            bool tieneAltura = double.TryParse(TxtAltura.Text, out double c) && c > 0;
            ...
            if (valoresIngresados == 2)
            {
                ...
                if (tieneAncho && tieneLargo && !tieneAltura)
                {
                    ...
                    recomendacion = $"Altura (c) debe estar entre {minAltura:F2} y {maxAltura:F2}";
                }
                else if (tieneAncho && tieneAltura && !tieneLargo)
                {
                    double minLargo = Math.Abs(a - c);
                    double maxLargo = a + c;
                    recomendacion = $"Largo (b) debe estar entre {minLargo:F2} y {maxLargo:F2}";
                }
                else if (tieneLargo && tieneAltura && !tieneAncho)
                {
                    double minAncho = Math.Abs(b - c);
                    double maxAncho = b + c;
                    recomendacion = $"Ancho (a) debe estar entre {minAncho:F2} y {maxAncho:F2}";
                }
        ...
        {
            if (!double.TryParse(TxtAncho.Text, out lado1) || lado1 <= 0)
            {
                MessageBox.Show("Lado (a) debe ser un número válido mayor a 0");
                return;
            }

            if (!double.TryParse(TxtLargo.Text, out lado2) || lado2 <= 0)
            {
            ...
            if (string.IsNullOrWhiteSpace(TxtAltura.Text))
            {
                ...
                TxtAltura.Text = lado3.ToString("F2");
            }
            else
            {
                if (!double.TryParse(TxtAltura.Text, out lado3) || lado3 <= 0)
                {
                    MessageBox.Show("Altura (c) debe ser un número válido mayor a 0");
                    return;
                }
            }
        ...
        private void FrmTriangle_Paint(object sender, PaintEventArgs e)
        {
